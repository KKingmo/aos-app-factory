...
class MediaPlayerService : Service() {
    ...
    override fun onCreate() {
        ...
        val playPendingIntent = PendingIntent.getService(
            baseContext, 0, Intent(baseContext, MediaPlayerService::class.java).apply {
        ...
        val notification = Notification.Builder(baseContext, CHANNEL_ID)
            ...
            .setSmallIcon(R.drawable.baseline_music_note_24).addAction(
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            MEDIA_PLAYER_PLAY -> {
                if (mediaPlayer == null) {
                    mediaPlayer = MediaPlayer.create(baseContext, R.raw.bgm)
                }
                mediaPlayer?.start()
            }
            MEDIA_PLAYER_PAUSE -> {
